// Preserved planning-baseline checks plus current document banners.
// Approval is tested separately; no production selection, classification or calculation.
...
const manifest = json("SOURCE_MANIFEST.reviewed-v0.4.json");
const golden = json("GOLDEN_CASE_CANDIDATE.json");
const activation = json("ACTIVATION_RECORD.reviewed-v0.4.json");
...
test("current controls are active; reviewed planning snapshots remain unchanged", () => {
  for (const name of [
    "SPRINT.md",
    "FRAMEWORK_SCOPE.md",
...
test("activation record is preserved and source review distinguishes readiness from approval", () => {
  ...
  const bytes = Buffer.from(
    read("ACTIVATION_RECORD.reviewed-v0.4.json").replaceAll("\r\n", "\n"),
    "utf8",
  );
  assert.equal(
    createHash("sha256").update(bytes).digest("hex"),
